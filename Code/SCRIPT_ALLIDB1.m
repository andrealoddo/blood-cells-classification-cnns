...
saveTables = 1;
saveTablesCross = 1;
saveTablesALL = 0; %one table per dataset and training version containg same and cross experiments
saveTablesBBvsMask = 0; %one table per dataset containg same experiments ob both BB and mask versions
savePGFPlotTable = 1;
...
%------------------------------------------------------------------------------------------------------------
%Compute one table per dataset containg same-version experiments on both BB and mask versions
if saveTablesBBvsMask == 1
    headings =  ['\\documentclass[12pt,italian]{article}\n',...
        '\\usepackage{graphicx}\n',...
        '\\usepackage{longtable}\n',...
        '\\parskip 0.1in\n',...
    ...
    splits = {{'img','img_tCrop','img_mask','img_tMask'}, ...
        {'img','img_tCrop','img_mask','img_tMask'}};

    for dt = 1:numel(datasets)
        destinationPerf = fullfile(perfPath,...
        strcat('PerformancesBBvsMask___',...
        datasetsname{dt}, '.tex'));
        pFile = fopen(destinationPerf, 'w');
        fprintf(pFile, headings);
        for cla = 1:numel(classifier)
                        ...
                        for pp = 1:numel(prepro)
                            for pop = 1:numel(postpro)
                                ...
                                for dsc_set = 1:numel(descriptors_sets)
                                    ...
                                    fprintf(pFile, '& %s ', desc_write);
                                    for targetsp = 1:numel(splits{dt})%target split
%                                        if contains( splits{dt}{targetsp}, 'ask' )
                                            clPATH = classifPath;
%                                        else
%                                            clPATH = '/home/lputzu/Workspaces/MATLAB/Esperimenti/Medical/Blood/blood-cells-classification-cnns/ClassificationOLD';
%                                        end
                                        destinationResult = [clPATH '/',...
                                            datasetsname{dt} '___',...
...
if savePGFPlotTable == 1
   
    training_splits = {{'img','img_tCrop'}, {'img_mask','img_tMask'}};
    splits = {{'img','img_tCrop','img_wrongCrop','img_wrongCrop2'},{'img_mask','img_tMask','img_wrongMask','img_wrongMask2','img_tWrongMask','img_tWrongMask2'}};
    postpro = ["none"];
    
    for dt = 1:numel(datasets)
        for type = 1:numel(training_splits)%ROI type, BB or BM
            for sourcesp = 1:numel(training_splits{type})%source split
                string_split = [training_splits{type}{sourcesp} 'TOothers___'];
                destinationPerf = fullfile(perfPath,...
                    strcat('PerformancesPlot___',...
                    datasetsname{dt}, '___',...
                    string_split, '.tex'));
                pFile = fopen(destinationPerf, 'w');
                for cla = 1:numel(classifier)
                        ...
                        for sel = 1:numel(selection)
                            ...
                            for gl = 1:numel(graylevel)
                                for pp = 1:numel(prepro)
                                    for pop = 1:numel(postpro)
                                        %Write table
                                        fprintf(pFile, '\\pgfplotstableread{\n');
                                        for dsc_set = 1:numel(descriptors_sets)
                                            if contains( descriptors_sets{dsc_set}, '_' )
                                                desc_write = erase( descriptors_sets_names{dsc_set}, '_' );
                                            else
                                            ...
                                            fprintf(pFile, ' %s ', desc_write);
                                            for targetsp = 1:numel(splits{type})%target split
                                                if sourcesp ~= targetsp
                                                    string_split = [splits{type}{sourcesp} 'TO' splits{type}{targetsp} '___'];
                                                    destinationResult = [classifPath '/Cross___',...
                                                        datasetsname{dt} '___',...
                                                        string_split ,...
                                                        descriptors_sets{dsc_set} '___',...
                                                    ...
                                                    string_split = splits{type}{sourcesp};
                                                    destinationResult = [classifPath '/',...
                                                        datasetsname{dt} '___',...
                                                        string_split '___',...
                                                        descriptors_sets{dsc_set} '___',...
                                        ...
                                        fprintf(pFile, '}\\testdata');
                                        fprintf(pFile, classifier{cla});
                                        fprintf(pFile, '\n');
                                    end
                                end
                            end
