function computePerformancesExec( datasets, datasetsname, training_splits, ...
    ...
    for dt = 1:numel(datasets)
        ...
        for sourcesp = 1:numel(training_splits)         % training splits
            for targetsp = 1:numel(splits{dt})            % all splits
                
                testLabels=[];
                [trainLabels, string_split, idx, trainSplit] = loadLabels( datasetsname{dt}, labelsPath, splits{dt}, targetsp);
                    ...
                    for gl = 1:numel(graylevel)
                        for pp = 1:numel(prepro)
                            ...
                            for dsc = 1:numel(descriptors)
                                [DBTrain_one, DBTest_one] = getFeatures( datasetsname{dt}, featsPath, ...
                                        ...
                                        for pop = 1:numel(postpro)
                                            ...
                                            if isfile( destinationResult ) == 0
                                                ...
                                                [model,wAVG] = EvaluateCrossClassification(DBTrain, ...
                                                    ...
                                                    postpro{pop}, selected_all, classifier{cla}, idx);
